...
struct romlist **getromlistbyident(int ver, int rev, int subver, int subrev, TCHAR *model, int all)
{
	...
	if (all)
		...
		max = romlist_cnt;
	buf = malloc ((sizeof (struct romlist*) + sizeof (struct romlist)) * (i + 1));
...
struct romlist **getarcadiaroms(void)
{
	...
	buf = malloc ((sizeof (struct romlist*) + sizeof (struct romlist)) * (max + 1));
	rdout = (struct romlist**)buf;
...
static void addkey (uae_u8 *key, int size, const TCHAR *name)
{
	...
	for (i = 0; i < ROM_KEY_NUM; i++) {
		if (keyring[i].key && keyring[i].size == size && !memcmp (keyring[i].key, key, size)) {
			free (key);
			//write_log ("key already in keyring\n");
			return;
		}
	}
	for (i = 0; i < ROM_KEY_NUM; i++) {
...
int load_keyring (struct uae_prefs *p, TCHAR *path)
{
	...
	for (;;) {
		...
		case 4:
			...
			break;
		case 5:
			_stprintf (tmp, "%s../shared/rom/rom.key", "./");
			break;
		case 6:
			if (p) {
...
struct romdata *getromdatabyzfile (FILE *f)
{
	int pos, size;
	uae_u8 *p;
	struct romdata *rd;
...
void getromname	(struct romdata *rd, TCHAR *name)
{
	...
	if ((rd->subrev || rd->subver) && rd->subver != rd->ver)
		_stprintf (name + _tcslen (name), " rev %d.%d", rd->subver, rd->subrev);
	if (rd->size > 0)
		_stprintf (name + _tcslen (name), " (%dk)", (rd->size + 1023) / 1024);
...
{
    if (uae_get_state () == UAE_STATE_RUNNING || quit_program == 0) {
...
void a3000_fakekick (int map)
{
	...
	if (map) {
		...
		if (fkickmemory[2] == 0x4e && fkickmemory[3] == 0xf9 && fkickmemory[4] == 0x00) {
			if (!kickstore)
				kickstore = malloc (fkickmem_size);
...
static int read_rom_file (uae_u8 *buf, struct romdata *rd)
{
	...
	if (!rl || _tcslen (rl->path) == 0)
		return 0;
	zf = zfile_fopen (rl->path, "rb", ZFD_NORMAL);
	if (!zf)
		return 0;
	addkeydir (rl->path);
...
struct zfile *read_rom_name (const TCHAR *filename)
{
	...
	f = rom_fopen (filename, "rb", ZFD_NORMAL);
	if (f) {
		uae_u8 tmp[11];
...
#define USE_UAE_ERSATZ "USE_UAE_ERSATZ"
	...
	if (f != NULL) {
		...
		kickmem_mask = size - 1;
		kickmem_size = size;
#if defined CDTV || defined CD32 || ARCADIA
