...
static void set_chipset_state (void)
{
    ...
    chipsetspeedpanel_set_leds_on_screen  (CHIPSETSPEEDPANEL (chipsetspeed_panel), currprefs.leds_on_screen);
}
...
static void joy_changed (void)
{
    if (! gui_active)
	return;
    changed_prefs.jport0 = map_widget_to_jsem (find_current_toggle (joy_widget[0], JOY_WIDGET_COUNT));
    changed_prefs.jport1 = map_widget_to_jsem (find_current_toggle (joy_widget[1], JOY_WIDGET_COUNT));
...
static void on_leds_on_screen_changed (void)
{
    currprefs.leds_on_screen = CHIPSETSPEEDPANEL (chipsetspeed_panel)->leds_on_screen;
...
static void make_chipset_widgets (GtkWidget *vbox)
{
    ...
    gtk_signal_connect (GTK_OBJECT (chipsetspeed_panel), "leds-on-screen-changed", GTK_SIGNAL_FUNC (on_leds_on_screen_changed), NULL);
}
