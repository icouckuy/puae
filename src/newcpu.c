...
static void build_cpufunctbl (void)
{
	...
	opcnt, lvl, currprefs.cpu_cycle_exact ? -1 : currprefs.cpu_compatible ? 1 : 0, currprefs.address_space_24);
    write_log ("CPU=%d, FPU=%d, JIT%s=%d.\n",
	currprefs.cpu_model, currprefs.fpu_model,
	currprefs.cachesize ? (currprefs.compfpu ? "=CPU/FPU" : "=CPU") : "", currprefs.cachesize);
    build_comp ();
#else
    write_log ("Building CPU, %d opcodes (%d %d %d)\n",
        opcnt, lvl, currprefs.cpu_cycle_exact ? -1 : currprefs.cpu_compatible ? 1 : 0, currprefs.address_space_24);
    write_log ("CPU=%d, FPU=%d\n",
        currprefs.cpu_model, currprefs.fpu_model);
#endif
    set_cpu_caches ();
}
...
static void m68k_disasm_2 (void *f, uaecptr addr, uaecptr *nextpc, int cnt, uae_u32 *seaddr, uae_u32 *deaddr, int safemode)
{
	...
	f_out (f, "%08x ", m68k_getpc () + m68kpc_offset);
	m68kpc_offset += 2;
...
void m68k_do_rte (struct regstruct *regs, uae_u32 pc, uae_u16 sr, uae_u16 format, uae_u16 opcode)
{
	int f;
